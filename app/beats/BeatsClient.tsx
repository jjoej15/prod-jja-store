...
export default function BeatsClient({ beats }: Props) {
    ...
    function skipBack() {
        ...
        const now = Date.now()
        // If second click within threshold -> previous track
        if (now - lastBackClickRef.current < DOUBLE_BACK_THRESHOLD_MS) {
            ...
            // Persist position of current track before switching
            if (currentBeat) {
                setPositions(p => ({ ...p, [currentBeat.id]: audio.currentTime }))
            }
    ...
    return (
        <div className="relative pb-28">
            <BeatsList
                ...
                onToggle={toggleBeat}
            />
            <div className="fixed bottom-0 left-0 right-0 bg-zinc-900/90 text-white backdrop-blur border-t border-zinc-800 px-4 py-3 shadow-sm">
                <div className="flex items-center justify-between gap-4 md:grid md:grid-cols-[12rem_1fr_6rem] md:items-center">
                    {/* Left: Name info (always visible) */}
                    <div className="w-48 overflow-hidden">
                        ...
                        <div className="text-xs text-zinc-300 truncate">
                            {currentBeat ? (currentBeat.artists.join(', ').replace('jj.aholics', 'Joe Anderson') || 'Unknown Artist') : '—'}
                        </div>
                    </div>

                    {/* Center: Mobile shows only controls; md+ shows slider + timestamp + controls */}
                    <div className="flex items-center gap-3 w-auto md:w-full px-2 md:col-start-2 md:col-end-3">
                        {/* Slider & timestamp hidden on small screens */}
                        <div className="hidden md:flex items-center gap-3 w-full">
                            <input
                                ...
                                style={{
                                    background: `linear-gradient(to right, #27272a ${progressPercent}%, #e5e7eb ${progressPercent}%)`
                                }}
                                aria-label="Seek"
                            />
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                            ...
                            >
                                <Image src={backwardStepIcon} alt="Restart" width={20} height={20} className="w-[20px] h-[20px] shrink-0 hover:cursor-pointer" />
                            </button>
                            <button
                                onClick={togglePlayPause}
                                className="p-2 rounded border border-zinc-700 hover:bg-zinc-800 flex items-center justify-center shrink-0"
                                aria-label={isPlaying ? 'Pause' : 'Play'}
                            >
                                <Image src={isPlaying ? pauseIcon : playIcon} alt={isPlaying ? 'Pause' : 'Play'} width={24} height={24} className="w-[24px] h-[24px] shrink-0 hover:cursor-pointer" />
                            </button>
                            <button
                                onClick={skipForward}
                                className="p-2 rounded border border-zinc-700 hover:bg-zinc-800 shrink-0"
                            ...
                            >
                                <Image src={forwardStepIcon} alt="Next" width={20} height={20} className="w-[20px] h-[20px] shrink-0 hover:cursor-pointer" />
                            </button>
                        </div>
                    </div>

                    {/* Right: Price info hidden on small screens */}
